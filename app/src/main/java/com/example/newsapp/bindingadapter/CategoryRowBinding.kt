...
import com.example.newsapp.ui.categories.CategoriesListFragmentDirections
    ...
    companion object {
        ...
        fun onCategoryClickListener(newsRowLayout: ConstraintLayout, article: Article, action: NavAction) {
            newsRowLayout.setOnClickListener {
                try {
                    val action =
                        CategoriesListFragmentDirections.actionCategoriesListFragmentToNewsDetailFragment(
                            article
                        )
                    newsRowLayout.findNavController().navigate(action)

                    Log.d("CategoryRowBinding", article.urlToImage!!)
                } catch (e: Exception) {
